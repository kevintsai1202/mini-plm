...
//	@Query(value = "select * from MP_ACTION where type = 'A' and user_id = ?1 and finish_flag != 1", nativeQuery = true)
	...
	@Query(value = "select count(*) from MP_ACTION where form_id = ?1 and finish_flag != 1" , nativeQuery = true)
	public int countAllUnfinishActions(Long formId);
	
	@Query(value = "select * from MP_ACTION where form_id = ?1 order by id desc", nativeQuery = true)
	public List<Action> findByFormId(Long formId);
	
	@Query(value = "select * from MP_ACTION where form_id = ?1 and type = ?2 order by id desc", nativeQuery = true)
	public List<Action> findByFormIdAndType(Long formId, String type);
	
	@Modifying
	@Query(value = "update MP_ACTION set FINISH_FLAG = 1 where FINISH_FLAG = 0 and form_id = ?1" , nativeQuery = true)
	public void updateAllUnfinishActions(Long formId);
	
	@Modifying
	@Query(value = "update MP_ACTION set FINISH_FLAG = 1 where FINISH_FLAG = 0 and form_id = ?1 and config_step_id = ?2" , nativeQuery = true)
	public void updateCurrUnfinishActions(Long formId, Long configStepId);
	
	@Modifying
	@Query(value = "update MP_ACTION set CAN_NOTICE_FLAG = 1 where FINISH_FLAG = 0 and CAN_NOTICE_FLAG = 0 and form_id = ?1 and config_step_id = ?2" , nativeQuery = true)
